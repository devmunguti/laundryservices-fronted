...
export default function AdminPromotionsManagement({ isStandalone = true }) {
  ...
  const [rejectReason, setRejectReason] = useState('M-Pesa transaction code could not be verified.');
  ...
  const handleConfirmApprove = async () => {
    ...
    try {
      ...
      const res = await promotionApi.approvePromotion(approveTarget._id);
      if (res.success) {
        toast.success(`Promotion for ${approveTarget.providerName} approved & activated!`);
        setApproveTarget(null);
        await fetchPromotionsAndSettings();
      } else {
        toast.error(res.message || 'Failed to approve promotion.');
      }
    } catch (err) {
      toast.error(err.response?.data?.message || 'Error approving promotion.');
    } finally {
  ...
  const handleConfirmReject = async () => {
    ...
    try {
      ...
      const res = await promotionApi.rejectPromotion(rejectTarget._id, { reason: rejectReason });
      if (res.success) {
        toast.success(`Promotion claim for ${rejectTarget.providerName} marked as rejected.`);
        setRejectTarget(null);
        await fetchPromotionsAndSettings();
      } else {
        toast.error(res.message || 'Failed to reject promotion.');
      }
    } catch (err) {
      toast.error(err.response?.data?.message || 'Error rejecting promotion.');
    } finally {
      setProcessingId(null);
    }
  };
  ...
  const handleSaveSettings = async (e) => {
    ...
    try {
      ...
      if (res.success) {
        toast.success('Promotion Paybill and receiving settings updated successfully!');
        setIsSettingsOpen(false);
        await fetchPromotionsAndSettings();
      } else {
        toast.error(res.message || 'Failed to update promotion settings.');
      }
    } catch (err) {
  ...
  return (
    <div className="space-y-6">
      ...
      {/* Confirmation Modal for Promotion Approval */}
      <ConfirmationModal
        isOpen={Boolean(approveTarget)}
        onClose={() => setApproveTarget(null)}
        onConfirm={handleConfirmApprove}
        title="Approve Promotion Spot"
        itemName={approveTarget ? `${approveTarget.providerName} - ${approveTarget.packageName}` : ''}
        warningMessage={`Verify and activate the featured placement spot for ${approveTarget?.providerName}. The provider's rating badge and top listing will be activated immediately.`}
        confirmText="Approve & Activate Spot"
        type="primary"
        isLoading={processingId === approveTarget?._id}
      />
        ...
        <div className="fixed inset-0 bg-black/40 backdrop-blur-xs z-50 flex items-center justify-center p-4">
          <div className="bg-white rounded-2xl shadow-xl max-w-md w-full p-6 space-y-4">
            <div className="flex items-center gap-3 text-rose-600">
              ...
              <h3 className="font-bold text-lg text-slate-900">Reject Promotion Claim</h3>
            </div>
            <p className="text-sm text-slate-600">
              Rejecting claim from <span className="font-bold text-slate-900">{rejectTarget.providerName}</span> (M-Pesa: {rejectTarget.mpesaTransactionCode}).
            </p>
            <div>
              <label className="block text-xs font-bold text-slate-700 uppercase tracking-wider mb-1">Reason for Rejection</label>
              <textarea
                value={rejectReason}
                onChange={(e) => setRejectReason(e.target.value)}
                className="w-full bg-slate-50 border border-slate-200 rounded-xl p-3 text-sm text-slate-800 outline-none focus:ring-2 focus:ring-rose-500/30"
                rows={3}
              />
            </div>
            <div className="flex gap-2 justify-end pt-2 border-t border-slate-100">
              ...
              >
                {processingId === rejectTarget._id ? 'Rejecting...' : 'Confirm Rejection'}
              </button>
            </div>
          </div>
        </div>
